...
public class LoopDetector implements Detector {
    public void analyze(Block block) {
        ...
        while (block.hasMoreOperations()) {
            ...
            // Inner loop
            if (priorTarget == null)
                priorTarget = block.getLastOperation();   // TODO better detection of target operation
    ...
    private void createForwardLoop(Block block, IfView firstIf) {
        ...
        if (firstPriorOp instanceof GoToView && block.getOperationByStartByte(((GoToView) firstPriorOp).getTargetOperation()) == null)
            return;
